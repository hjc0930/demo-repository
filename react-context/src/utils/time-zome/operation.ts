...
export const UTC_getYear = (datePara?: _Date) =>
  (datePara ? new Date(datePara as _Date) : new Date()).getUTCFullYear();
export const UTC_getMonth = (datePara?: _Date) =>
  (datePara ? new Date(datePara as _Date) : new Date()).getUTCMonth();
export const UTC_getDate = (datePara?: _Date) =>
  (datePara ? new Date(datePara as _Date) : new Date()).getUTCDate();
export const UTC_getHours = (datePara?: _Date) =>
  (datePara ? new Date(datePara as _Date) : new Date()).getUTCHours();
export const UTC_getMinutes = (datePara?: _Date) =>
  (datePara ? new Date(datePara as _Date) : new Date()).getUTCMinutes();
export const UTC_getSeconds = (datePara?: _Date) =>
  (datePara ? new Date(datePara as _Date) : new Date()).getUTCSeconds();

/** @以下是获取指定时区的指定数据 */
// 不传则默认代码运行环境时的当前时区
export const get_mmddyyyy = (
  region: string,
  symbol?: string,
  datePara?: DatePara
) => {
...
export const custom_yyyymmdd_sort = (
  region: string,
  ymdsort?: string,
  symbol?: string,
  datePara?: DatePara
...
export const getESTmmddyyyy = (datePara?: _Date) => {
  const estDate = getESTDate(datePara);
  const result = estDate.split(" ")[1].replace(",", ""); // 月 日 年
  return result;
};
