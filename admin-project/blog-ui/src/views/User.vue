...
const onGetTableData = async (pageNum = 1) => {
  try {
    loading.value = true;
    const result = await fetch(`http://localhost:8080/user/page?pageNumber=${pageNum}&pageSize=${pageSize}`).then(res => res.json())
...
const onDelete = (row) => {
  ElMessageBox.confirm(
      'Are your sure deleted this user?',
      'Warning',
      {
...
<template>
  <div>
    <el-button type="primary" @click="dialogTableVisible = true">Add User</el-button>
    <ElDialog v-model="dialogTableVisible" title="Add User" @close="onDialogClose">
      <el-form :model="form">
